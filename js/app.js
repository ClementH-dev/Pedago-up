...
function initBannerCarousel({
  ...
  const root = document.querySelector(rootSelector);
  const viewport = document.getElementById(viewportId);
  const bannerItems = root?.querySelectorAll('.network-card') ?? [];
  const bannerDots = document.querySelectorAll(`#${dotsId} .dot`);
  const prev = document.getElementById(prevId);
  const next = document.getElementById(nextId);
  const autoToggle = document.getElementById(toggleId);
  let currentBanner = 0;
  ...
  let isBannerManualPaused = true;
  let isBannerInteractionPaused = false;
  ...
  function updateBannerControls() {
    if (!bannerItems.length || !prev || !next) return;
    const hasOverflow = viewport ? viewport.scrollWidth > viewport.clientWidth + 1 : bannerItems.length > 1;
    prev.disabled = !hasOverflow;
    next.disabled = !hasOverflow;
  }

  function updateBannerToggle() {
    if (!autoToggle) return;
    if (reducedMotion.matches) {
      autoToggle.disabled = true;
      autoToggle.setAttribute('aria-pressed', 'true');
    ...
    autoToggle.disabled = bannerItems.length < 2;
    if (isBannerManualPaused) {
      autoToggle.setAttribute('aria-pressed', 'true');
      autoToggle.textContent = startText;
      return;
    }
    ...
    autoToggle.setAttribute('aria-pressed', 'false');
    autoToggle.textContent = isBannerInteractionPaused ? suspendedText : pauseText;
  }

  function setActiveBanner(index, { announce = false } = {}) {
    ...
    bannerItems.forEach((item, itemIndex) => {
      item.classList.toggle('active', itemIndex === currentBanner);
      item.removeAttribute('aria-hidden');
    });

    bannerDots.forEach((dot, dotIndex) => {
      dot.classList.toggle('active', dotIndex === currentBanner);
      dot.setAttribute('aria-pressed', dotIndex === currentBanner ? 'true' : 'false');
    });
  }

  function scrollToBanner(index, { announce = false } = {}) {
    if (!viewport || !bannerItems.length) return;

    const targetIndex = (index + bannerItems.length) % bannerItems.length;
    const target = bannerItems[targetIndex];
    const left = target.offsetLeft - ((viewport.clientWidth - target.clientWidth) / 2);

    setActiveBanner(targetIndex, { announce });
    viewport.scrollTo({
  ...
  function getClosestBannerIndex() {
    if (!viewport || !bannerItems.length) return currentBanner;
  ...
  function startBannerAuto() {
    ...
    if (
      ...
      isBannerManualPaused ||
      isBannerInteractionPaused ||
      reducedMotion.matches
    ) {
      return;
    }
    ...
    bannerTimer = setInterval(
      () => scrollToBanner(currentBanner + 1),
      CARD_CAROUSEL_AUTO_DELAY_MS
    );
  }

  bannerDots.forEach(dot => {
    dot.addEventListener('click', () => {
      isBannerManualPaused = true;
      stopBannerAuto();
      updateBannerToggle();
      scrollToBanner(Number(dot.dataset.index), { announce: true });
    });
  });
  ...
  prev?.addEventListener('click', () => {
    ...
    stopBannerAuto();
    updateBannerToggle();
    scrollToBanner(currentBanner - 1, { announce: true });
  });
  ...
  autoToggle?.addEventListener('click', () => {
    if (reducedMotion.matches || bannerItems.length < 2) return;

    isBannerManualPaused = !isBannerManualPaused;
    updateBannerToggle();
    if (isBannerManualPaused) {
      stopBannerAuto();
    } else {
  ...
  viewport?.addEventListener('scroll', () => {
    window.clearTimeout(bannerScrollTimeout);
    bannerScrollTimeout = window.setTimeout(() => {
      setActiveBanner(getClosestBannerIndex());
    }, 120);
  });

  root?.addEventListener('focusin', () => {
    isBannerInteractionPaused = true;
    stopBannerAuto();
    updateBannerToggle();
  });
  ...
  root?.addEventListener('focusout', event => {
    if (!event.currentTarget.contains(event.relatedTarget)) {
      isBannerInteractionPaused = false;
      updateBannerToggle();
      startBannerAuto();
    }
  });
  ...
  window.addEventListener('load', updateBannerControls);
